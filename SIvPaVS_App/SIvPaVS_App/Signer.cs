...
namespace SIvPaVS_App
{
    ...
    { 
        ...
        public string SignXml() {
            ...
            var signerObj = signer.CreateObject("receipts" + (counter++).ToString(), "Pokladničný blok", xml, Resources.receipts, "", a + @"\..\..\Resources\receipts.xsd", Resources.XML_to_TXT_XSLT, a + @"\..\..\Resources\XML_to_TXT_XSLT.xsl");


            Ditec_Zep_DSigXadesAtl.CXadesSigAtl oXML = new Ditec_Zep_DSigXadesAtl.CXadesSigAtl();

            oXML.AddObject(signerObj);
            signerObj = signer.CreateObject("receipts"+(counter++).ToString(), "Pokladničný blok", xml, Resources.receipts, "", a + @"\..\..\Resources\receipts.xsd", Resources.XML_to_TXT_XSLT, a + @"\..\..\Resources\XML_to_TXT_XSLT.xsl");
            oXML.AddObject(signerObj);
